...
def parseFile(filename):
    ...
    for line in f:
    	lst = re.split(r'\t+', line)
	similarities[(lst[0], lst[1])] = float(lst[2])
...
def linSimilarities(word_pairs, ic):
    ...
    for wp in word_pairs:
    	l1 = wn.synsets(wp[0], pos=wn.NOUN)
	if len(l1) == 0:
	    l1 = wn.synsets(wp[0], pos=wn.VERB)
	    if len(l1) == 0:
	    	print "no noun or verb synset found for [%s]." % wp[0]
		continue
	    else:
	    	l2 = wn.synsets(wp[1], pos=wn.VERB)
		if len(l2) == 0:
		    print "no matching pos for words: [%s] and [%s]" % (wp[0], wp[1])
		    continue
	else:
	    l2 = wn.synsets(wp[1], pos=wn.NOUN)
	    if len(l2) == 0:
	    	l2 = wn.synsets(wp[1], pos=wn.VERB)
		if len(l2) == 0:
	    	    print "no noun or verb synset found for [%s]." % wp[1]
		    continue
	        else:
	    	    l1 = wn.synsets(wp[0], pos=wn.VERB)
		    if len(l1) == 0:
	...
	w1 = l1[0]
    	w2 = l2[0]
	s = float(w1.lin_similarity(w2, ic))
	similarities[wp] = s * 10
...
def resSimilarities(word_pairs, ic):
	...
	w1 = l1[0]
    	w2 = l2[0]
	similarities[wp] = w1.res_similarity(w2, ic)
...
def vecSimilarities(word_pairs, model):
    ...
    for wp in word_pairs:
        similarities[wp] = float(model.similarity(wp[0].lower(), wp[1].lower())) * 10
